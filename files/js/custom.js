...
$(document).ready(function () {
  ...
  var files = [
    {
      url:
          "http://www.4-traders.com/BLOCKCHAIN-GROUP-CO-LTD-6165838/news/Blockchain-LegalThings-to-digitise-law-on-Blockchain-25681815/",
      img: "4-traders.png"
    },
    {
      url:
          "https://advocatenblad.nl/2017/10/19/legalthings-one-smart-contracts/",
      img: "advocatenblad.png"
    },
    {
      url:
          "https://appdevelopermagazine.com/5755/2017/12/14/Blockchain-tech-to-help-digitize-Dutch-government-legislation/",
      img: "app-developer-magazine.png"
    },
    {
      url:
          "http://www.advocatie.nl/lexoo-legalthings-signrequest-naar-finale-legal-tech-startup-awards",
      img: "advocatie.png"
    },
    {
      url:
          "http://www.blockchainadvisormag.com/2017/12/16/ethereum-competitor-legalthings-to-digitise-law-on-blockchain-for-the-dutch-district-attorneys-office/",
      img: "blockchainadvisor.png"
    },
    {
      url:
          "https://btcmanager.com/dutch-ministry-justice-commissions-digitizes-law-blockchain/",
      img: "btc-manager.png"
    },
    {
      url:
          "https://contactlessintelligence.com/2017/12/20/dutch-tech-disruptor-releases-contract-of-the-future/",
      img: "contactless-intelligence.png"
    },
    {
      url:
          "https://dutchitchannel.nl/590090/merin-en-legalthings-lanceren-blockchain-toepassing-in-vastgoed.html",
      img: "dutch-it-channel.png"
    },
    {
      url:
          "https://www.emerce.nl/wire/legalthings-lanceert-succesvolle-token-sale-nederland-nederlandse-ethereumconcurrent-digitaliseert-wet-regelgeving-via-blockchain",
      img: "emerce.png"
    },
    {
      url: "https://mini.eastday.com/a/171220141936829.html",
      img: "english-eastday.png"
    },
    {
      url:
          "https://www.enterprisetimes.co.uk/2017/12/18/dutch-justice-seeks-to-digitize-on-legalthings-blockchain/",
      img: "enterprise-times.png"
    },
    {
      url:
          "http://www.elektroniknet.de/markt-technik/kommunikation/legalthings-digitalisiert-hollaendisches-gesetzgebung-148902.html",
      img: "markt-technik.png"
    },
    {
      url:
          "https://www.metronieuws.nl/xl/digitaal/2017/02/digitale-handtekening-dat-kan-veilig-met-blockchain",
      img: "metro.png"
    },
    {
      url:
          "https://www.nrc.nl/nieuws/2017/11/14/niet-alle-digitale-beursgangen-zijn-slecht-14017947-a1581143",
      img: "nrc.png"
    },
    {
      url:
          "https://www.publictechnology.net/articles/news/dutch-government-trial-blockchain-based-digital-law",
      img: "public-technology.png"
    },
    {
      url:
          "http://www.quotenet.nl/Nieuws/Amsterdamse-startup-gaat-Ethereum-slopen-Ether-is-een-scam-208104",
      img: "quote.png"
    },
    {
      url:
          "https://www.tahawultech.com/cnme/news/legalthings-digitise-law-blockchain/",
      img: "tahawul-tech.png"
    },
    {
      url:
          "http://www.vastgoedmarkt.nl/data-en-technologie/nieuws/2017/12/merin-zet-stap-blockchain-101128261",
      img: "vastgoedmarkt.png"
    },
    {
      url:
          "https://fd.nl/morgen/1171136/legaltech-minder-maatpak-meer-confectie",
      img: "fd.png"
    },
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    $(".media-slider").append(
        '<a href="' +
        files[i].url +
        '" target="_blank"><img style="margin: 16px;max-width: 150px;filter: grayscale(100%)" src="img/media/' +
        files[i].img +
        '"></a>'
    );
  }
  ...
  $("#checkout-total-price-options").on("change", function (newValue) {
    calculateFinalRate();
  });
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        $("#download-seed")
            ...
            .attr("download", "waves-wallet.txt");
        showCreateWallet();
      }
    });
  });
...
function getRates() {
  ...
  $.ajax({
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      $("#rates-message").text(
          "An error has occurred while trying to fetch the current rates. Please try again later."
      );
    }
  });
}
...
function calculateFinalRate() {
  ...
  var selectedCurrency = $("#checkout-total-price-options").val();
  console.log('---', selectedCurrency);
  var ltoAmount = $("#lto-amount").val();
  console.log('---', ltoAmount);
  ...
  if (currentCurrency) {
    ...
    var total = parseFloat(
        parseInt(ltoAmount) * parseFloat(currentCurrency.rate)
    ).toFixed(decimals);
    
    console.log('---', total);
    $("#price").val(total);
    $("#checkout-total-price").val(total);
    // $("#checkout-total-price-value").html(total);
    $("#checkout-total-price-value strong").html(total);
...
function handlePayment() {
  $("#pay").click(function () {
    ...
    var validWallet = checkWalletAddress(wallet, function (err, validWallet) {
      if (!validWallet) {
        $("#error-payment").html(
            "Invalid waves wallet address entered please correct it in previous step"
        );
        $("#error-payment").show();
        return;
      }
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (!result.transaction.id) {
        $("#error-payment").html(
            "Failed to complete transaction, please contact us on one of our social channels."
        );
        $("#error-payment").show();
      }
...
function convertUserToOrg(user) {
  ...
  organization.name =
      user.company == "" ? user.first_name + " " + user.last_name : user.company;
...
function loadCheckoutInformation() {
  ...
  $.ajax({
    ...
    success: function (result) {
      ltoRates = result;
      $("#checkout-total-price-options").html("");
      $.each(ltoRates, function (i, item) {
        $("#checkout-total-price-options").append(
            $("<option>", {
              value: item.currency,
              text: item.currency
            })
      ...
      $("select[name=checkout-total-price-options] option")
          .filter(function () {
            return $(this).text() == currency; //To select Blue
          })
          .prop("selected", true);
    }
  });
  ...
  $("#checkout-total-price-value").html("<strong>" + price + "</strong>");
  ...
  var explore = [
    {
      ...
      intro:
      "We show that the fundamental legal structure of a well-written financial contract follows a " +
    ...
    {
      link:
          "https://medium.com/@rick_42072/live-contracts-whats-all-the-fuss-about-62b167d9a4d4",
      img: "img/three_common.jpg",
      title: "Live Contracts — What’s all the fuss about?",
      intro:
      "Picture this. A contract or law visualized in such a way that it is understandable for humans and " +
    ...
    },
    {
      ...
      intro:
      "A Live Contract is an agreement that is formalized as a Finite State Machine. LegalThings One runs" +
  ...
  $.each(explore, function (key, value) {
    block.append(
        [
          ...
          '<a href="' +
          value.link +
          '" target="_blank" class="text-light blog-image">',
          "<img src=" +
          value.img +
          ' alt="SEO IMG NAME">' +
...
function loadTokens() {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      var cap = 10000000;
      var presaleLimit = parseFloat(
          result.phases.presale.limit / 1000000000,
          10
      );
  ...
  checkWalletAddress(address, function (err, valid) {
    if (!valid) {
      $("#wallet")
          .parsley()
          .addError("required", {
            message: "Invalid Waves wallet address",
            updateClass: true
          });
    } else {
      $("#wallet")
...
function disableNextToDownloadSeed() {
  ...
  if (!$(".sf-controls span").length) {
    $(".sf-controls").append(
        '<span class="parsley-required span-seed-msg">Please download your seed to continue</span>'
    );
  }
...
function wizardInit() {
  sfw = $("#wizard").stepFormWizard({
    ...
    onSlideChanged: function (to, data) {
      ...
      if ($(".sf-controls").find("span").length)
        $(".sf-controls")
            .find("span")
            .remove();
  ...
  $("#faucet-retrieve-btn").on("click", function (e) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        $("#faucet-failed").hide();
        $("#faucet-success").show();
        $("#faucet-success-link").attr(
            "href",
            "https://wavesexplorer.com/tx/" + result.id
        );
...
function initSubscribeMailChimp() {
  ...
  $(".newsletter-form").ajaxChimp({
    ...
    url:
        "https://legalthings.us17.list-manage.com/subscribe/post?u=1508bdb96b4379a9aeb07c6e8&amp;id=6d17a10ae2"
  });
}
...
function stripeCheckout(data, price) {
  ...
  var handler = StripeCheckout.configure({
    ...
    image:
        "https://s3-eu-west-1.amazonaws.com/livecontracts/img/logo/icon-purple.png",
...
if ($("#checkout-presale").length) {
  document
      .getElementById("checkout-presale")
