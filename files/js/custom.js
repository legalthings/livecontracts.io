...
function stripeCheckoutInit() {
	...
	document.getElementById('checkout-presale-creditcard').addEventListener('click', function (e) {
		var $amount = $('#checkout-presale-amount');
		...
		handler.open({
			amount: amount * 25, // 1 LTO = $0.25
			token: function (token) {
				$.ajax({
					type: "POST",
					url: waves_server + "/api/payment",
					data: JSON.stringify({
						amount: amount,
						email: token.email,
						token: token.id,
					...
					success: function (result) {
						if (!result || !result.id) {
							// something went wrong
							$('#checkout-presale-failed').show();
							$('#checkout-presale-success').hide();
							return;
						}
						
						$('#checkout-presale-failed').hide();
						$('#checkout-presale-success').show();
					},
					error: function () {
						$('#checkout-presale-failed').show();
						$('#checkout-presale-success').hide();
